...
void sparse_multiply(
    ...
    for(int i = 0 ; i < rows ; i++){
        ...
        for(int j = 0; j < cols ; j++){
            if(A[i * cols + j] != 0){
                col_indices[val_idx] = j;
                values[val_idx] = A[i*cols+j];
                val_idx++;
            }
        }
    }
    ...
    *out_nnz = val_idx;
    ...
    // print_CSR(values , col_index , row_ptr , rows , cols);

    for(int i = 0; i < rows ; i++){
        int start = row_ptrs[i];
        int end = row_ptrs[i+1];
        for(int val_id= start ; val_id < end ; val_id++){
            int j = col_indices[val_id];
            double val = values[val_id];
            y[i] += val * x[j];
        }
    }
}
